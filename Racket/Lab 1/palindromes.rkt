...
(define (all-palindromes? n Bases)
  (cond
    ((null? Bases) #t)
    ((list? Bases) (and (all-palindromes? n (car Bases)) (all-palindromes? n (cdr Bases))))
    (else (palindrome? (num->base n Bases)))))
...
(define (palindromes-to-n n Bases)
  (cond
       ...
       ((all-palindromes? n Bases)
           (cons n (palindromes-to-n (- n 1) Bases)))
