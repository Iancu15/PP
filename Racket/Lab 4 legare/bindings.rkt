...
(define (compute-length get-line-segment get-start-point get-end-point)
  (let* ((segment (get-line-segment)) (start-point (get-start-point segment))
                                      (end-point (get-end-point segment)))
    (sqrt (+ (sqr (- (car start-point) (car end-point)))
             (sqr (- (cdr start-point) (cdr end-point)))))))
...
(define (distance x y)
  (let ((get-segment (lambda ()
                       (cons x y)))
        (get-start-point (lambda (x)
                           (car x)))
        (get-end-point (lambda (x)
                         (cdr x))))
    (compute-length get-segment get-start-point get-end-point)))
...
(define (compute-f-with-step f a b step)
  (let iter ((start a) (end b) (acc '()))
    (if (> start end)
        acc
        (iter start (- end step) (cons (f end) acc)))))
...
(define (compute-perimeter points)
  (let iter ((list_points points)
             (perim (distance (car points) (car (reverse points)))))
    (if (null? (cdr list_points))
        perim
        (iter (cdr list_points) (+ perim (distance (car list_points)
                                                   (cadr list_points)))))))
