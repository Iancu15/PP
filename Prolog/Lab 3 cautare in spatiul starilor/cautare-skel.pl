...
state(_, _, _).
parseState(state(NM_Est, NC_Est, Mal), Mal, NM_Est, NC_Est, NM_Vest, NC_Vest) :- NM_Vest is (3 - NM_Est), NC_Vest is (3 - NM_Est).
...
initial_state(misionari, state(3, 3, est)).
...
final_state(misionari, state(0, 0, vest)).
...
get_new_composition(NM_Est1, NC_Est1, NMB, NCB, est, vest, NM_Est2, NC_Est2) :-
    NM_Est2 is (NM_Est1 - NMB), NC_Est2 is (NC_Est1 - NCB).

get_new_composition(NM_Est1, NC_Est1, NMB, NCB, vest, est, NM_Est2, NC_Est2) :-
    NM_Est2 is (NM_Est1 + NMB), NC_Est2 is (NC_Est1 + NCB).

is_boat_allright(NMB, NM_Est, NCB, NC_Est, est) :-
    NM_Est >= NMB, NC_Est >= NCB.

is_boat_allright(NMB, NM_Est, NCB, NC_Est, vest) :-
    parseState(state(NM_Est, NC_Est, _), _, _, _, NM_Vest, NC_Vest),
    NM_Vest >= NMB, NC_Vest >= NCB.

next_state(misionari, state(NM_Est1, NC_Est1, Mal1), state(NM_Est2, NC_Est2, Mal2)) :-
    boat(NMB, NCB), is_boat_allright(NMB, NM_Est1, NCB, NC_Est1, Mal1),
    get_new_composition(NM_Est1, NC_Est1, NMB, NCB, Mal1, Mal2, NM_Est2, NC_Est2),
    parseState(state(NM_Est2, NC_Est2, _), _, _, _, NM_Vest2, NC_Vest2),
    safeMisionari(NM_Est2, NC_Est2), safeMisionari(NM_Vest2, NC_Vest2).
