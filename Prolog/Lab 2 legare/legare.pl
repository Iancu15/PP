...
safe(X/_/S, [X/_/S | _]) :- !, false.
safe(_/Y/S, [_/Y/S | _]) :- !, false.
safe(X/Y/S, [_ | Others]) :- safe(X/Y/S, Others).
safe(_, []).
...
permutari4([x1, x2, x3, x4], Perm) :- length(Perm, 4), member(x1, Perm), member(x2, Perm),
                                    member(x3, Perm), member(x4, Perm).
...
%alternare_helper(X, Y, 0, Len, [X | Sol]) :- LenSub1 is (Len - 1), alternare_helper(X, Y, 1, LenSub1, Sol).
%alternare_helper(X, Y, 1, Len, [Y | Sol]) :- LenSub1 is (Len - 1), alternare_helper(X, Y, 0, LenSub1, Sol).
%alternare_helper(_, _, _, 0, []).
alternare(X, Y, Len, [X | Sol]) :- Len > 0, !, LenSub1 is (Len - 1), alternare(Y, X, LenSub1, Sol).
alternare(_, _, 0, []).
...
%% listei.  
sublista([E | L], [E | SL]) :- sublista(L, SL).
sublista([_ | L], SL) :- sublista(L, SL).
sublista([], []).
...
find2occ(L, X) :- append(L1, [X | L2], L), append(L1, L2, L3), append(L4, [X | L5], L3), append(L4, L5, L6), \+member(X, L6).
