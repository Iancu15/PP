...
au_marca(Marca, ListaNume) :- setof(Nume, Marca^conduce(Nume, Marca), ListaNume).
...
%% arme_bmw/1
%% arme_bmw(-ListaArme)
arme_bmw(ListaArme) :- au_marca(bmw, ListaNume), findall(Arma, (member(Nume, ListaNume), inarmat(Nume, Arma)), ListaArme).
...
arme_marca(Marca, ListaArme) :- au_marca(Marca, ListaNume), setof(Arma, ListaNume^Nume^(member(Nume, ListaNume), inarmat(Nume, Arma)), ListaArme).
