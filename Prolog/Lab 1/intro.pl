...
myConcat([], L, L).
myConcat([AP | AR], B, [AP | Res]) :- myConcat(AR, B, Res).
...
myReverse([], []).
myReverse([AP | AR], Rev) :- myConcat(FirstN, [AP], Rev), myReverse(AR, FirstN).
...
myReverseAcc([AP | AR], Acc, R) :- myReverseAcc(AR, [AP | Acc], R).
...
factorial(N, Fact) :- N > 0, M is N - 1, factorial(M, MF), Fact is N * MF.
