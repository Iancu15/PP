...
emptyBoard = Board (replicate 9 Neither)
...
place cell (x, y) (Board b) = Board (take n b ++ [cellstruct] ++ drop (n + 1) b)
    where
        cellstruct = if cell == 'X' then X else O
        n = 3 * x + y;
...
takeCell :: Int -> TicTacToe -> String
takeCell 0 (Board b) = show $ head $ take 1 b
takeCell x (Board b) = show $ head $ take 1 $ drop x b

instance Show TicTacToe
    where 
        show b = one ++ "\n" ++ two ++ "\n" ++ three where
            one = takeCell 0 b ++ takeCell 1 b ++ takeCell 2 b
            two = takeCell 3 b ++ takeCell 4 b ++ takeCell 5 b
            three = takeCell 6 b ++ takeCell 7 b ++ takeCell 8 b
...
moves :: [Cell] -> Int -> Int
moves [] nr = nr
moves b nr = moves (drop 1 b) (if cell == ' ' then nr else (nr + 1)) where
    cell = head $ show $ head $ take 1 b

instance Ord TicTacToe
    where 
        (Board b1) <= (Board b2) = (moves b1 0) <= (moves b2 0)
