...
numToBase n b = if (n == 0) then []
                            else (numToBase (n `quot` b) b) ++ [n `mod` b]
...
numToBase3 n b
    | n == 0 = []
    | otherwise = (numToBase (n `quot` b) b) ++ [n `mod` b]

-- case of
numToBase4 :: Integer -> Integer -> [Integer]
numToBase4 n b = case n == 0 of
    True -> []
    _    -> (numToBase (n `quot` b) b) ++ [n `mod` b]
...
newtonRaphson x g dg = infiniteApply (\x -> x - (g x) / (dg x))
