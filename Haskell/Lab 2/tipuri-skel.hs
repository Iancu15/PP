...
data BST a = BSTNode (BST a) a (BST a) | BSTNil deriving (Eq, Show)

isNull :: Eq a => BST a -> Bool
isNull node = (node == BSTNil)

makeLeaf :: a -> BST a
makeLeaf key = BSTNode BSTNil key BSTNil

insertElem :: (Ord a, Eq a) => BST a -> a -> BST a
insertElem BSTNil key = BSTNode BSTNil key BSTNil
insertElem (BSTNode rootleft rootvalue rootright) key
  | rootvalue < key = BSTNode rootleft rootvalue (insertElem rootright key)
  | otherwise = BSTNode (insertElem rootleft key) rootvalue rootright

findElem :: (Ord a, Eq a) => BST a -> a -> Maybe a
findElem BSTNil key = Nothing
findElem (BSTNode rootleft rootvalue rootright) key
  | rootvalue == key = Just key
  | rootvalue < key = findElem rootright key
  | otherwise = findElem rootleft key

inorder :: BST a -> [a]
inorder BSTNil = []
inorder (BSTNode BSTNil value BSTNil) = [value]
inorder (BSTNode rootleft rootvalue rootright) = (inorder rootleft) ++ [rootvalue] ++ (inorder rootright)

mapTree :: (a -> b) -> BST a -> BST b
mapTree func BSTNil = BSTNil
mapTree func (BSTNode rootleft rootvalue rootright) =
  (BSTNode (mapTree func rootleft) (func rootvalue) (mapTree func rootright))

foldlTree :: (b -> a -> b) -> b -> BST a -> b
foldlTree func acc BSTNil = acc
foldlTree func acc (BSTNode rootleft rootvalue rootright) =
  foldlTree func (foldlTree func (func acc rootvalue) rootleft) rootright
